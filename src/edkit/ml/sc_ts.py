...
class scdKit_ts():
  ...
  def grid(self,fLonStep,fLatStep):
    '''
	  产生grid节点
	'''
    xmin=70
  ...
  def plotMsCount(self,mStep=1.0,sSuffix="10",bShowFigure=True,bSaveFigure=True,bSaveData=True):
    '''
	震级区间内的地震数量，
	'''
    ms=[]
  ...
  def plotHourCount(self):
    ...
    print(u"hour list：",len(months),months)
    #print(len(dcs),dcs)
    ...
    for i in range(len(self.ds)):
      ...
      #print(self.ds3[i][scdFields['fieldHour']])
      idx=int( self.ds3[i][scdFields['fieldHour'] ])
      dcs[idx]=dcs[idx]+1
    ...
    #ok
    pl.plot(months,dcs,linewidth=4.0)
    #ok
    #pl.bar(months,dcs)
    pl.xlabel('Hour',fontsize=16)
  ...
  def plotDayCount(self):
    ...
    #ok
    #pl.bar(years,dcs)
    pl.xlabel('Day',fontsize=22)
    pl.ylabel('Count',fontsize=22)
    ymax=max(dcs)
    pl.ylim(0,ymax)
    pl.xticks([x+1 for x in range(31)])
    pl.gca().lines[0].set_color('r')
    #ok
    #pl.show()
  ...
  def plotMonthCount(self):
    ...
    ymax=max(dcs)
    pl.ylim(0,ymax)
    pl.xticks([x+1 for x in range(12)])
    #ok
    #pl.show()
